...
//
// backend.add(import('@mantine/dates'));
// auth
        ...
        providers.registerProvider({
          ...
          factory: createOAuthProviderFactory({
            ...
            async signInResolver(info, ctx) {
               ...
               *********************************************************************/
              console.log('HEREEE');
              // info.profile.email = 'frenky.tay01@gmail.com';
              console.log(info);
              let {
                profile: { email },
              } = info;
              ...
              // console.log(info.result.session.accessToken)
              if (!email) {
                const response = await fetch(
                  'https://api.github.com/user/emails',
                  {
                    headers: {
                      Authorization: `token ${info.result.session.accessToken}`,
                    },
                  },
                );
                ...
                if (!primaryEmail) {
                  throw new Error(
                    'User profile contained no email, and no primary email found.',
                  );
                }
              ...
              // const userId = info.profile.displayName;
